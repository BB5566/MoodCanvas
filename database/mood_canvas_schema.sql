...
(999, 'demo', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'demo@moodcanvas.com');
-- 預設密碼是 'password'

-- 插入範例日記 (Demo模式用)
-- 注意：原先使用的 `quote` 欄位在表結構中不存在，改為使用 `ai_generated_text` 欄位
INSERT IGNORE INTO diaries (user_id, title, content, mood, diary_date, ai_generated_text) VALUES 
(999, '歡迎使用 MoodCanvas', '這是一個展示用的日記條目。MoodCanvas 結合了傳統日記寫作與現代AI技術，為您的回憶增添藝術色彩。', '😊', CURDATE(), '「生活不在於等待風暴過去，而在於學會在雨中起舞。」'),
(999, '技術展示', '這個專案展示了PHP MVC架構、資料庫設計、API整合等多項技術能力。', '💻', DATE_SUB(CURDATE(), INTERVAL 1 DAY), '「程式設計不僅是科學，更是一門藝術。」'),
(999, '創作靈感', '結合AI與日記的想法來自於想要為平凡的文字記錄增添更多的創意元素。', '💡', DATE_SUB(CURDATE(), INTERVAL 2 DAY), '「創新來自於跨領域的思考碰撞。」');
